...
void CSurfaceMovement::SetHicksHenne(CGeometry *boundary, CConfig *config, unsigned short iDV, bool ResetDef) {
  ...
  AoA = 0.0;
	...
	cout << "HH - xk = " << xk << endl; cout << "chord = " << Chord << endl; 
        ek = log10(0.5)/log10(xk/Chord);
        if (Coord[0] > 10*EPS ) fk = pow( sin( PI_NUMBER * pow(Coord[0]/Chord, ek) ), t2);
        else fk = 0.0;
	cout << "Ampl = " << Ampl << " , fk = " << fk << endl;
	cout << "Ampl*fk = " << Ampl*fk << endl;
	if (Coord[0] == TPCoord[0])
	  {
	    fk = 0.0;
	    cout << "fk =0, Ampl*fk= " << Ampl*fk << endl;
	  }
