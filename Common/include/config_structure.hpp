...
private:
  ...
  *TemperatureLimits; /*!< \brief Limits for the primitive variables */
  ...
  unsigned short tempcoarse;  /*< \brief Number of levels of temporal coarsening for unsteady adjoint */
  su2double New_Elem_Adapt;			/*!< \brief Elements to adapt in the numerical grid adaptation process. */
  su2double Delta_UnstTime,			/*!< \brief Time step for unsteady computations. */
  Delta_UnstTimeND;						/*!< \brief Time step for unsteady computations (non dimensional). */
    
  su2double *NonUniform_timestps_ND; /* Array of non-uniform time step in non-dimensional form (array*timestep(dt) gives the non-uniform dt) */
  su2double *DiscAdjNonUniform_timestps_ND; /* Array of non-uniform time step in non-dimensional form (array*timestep(dt) gives the non-uniform dt) */
  unsigned long nNonUniformTS; /* Number of non-uniform time steps after the restart */
  bool NonUniform_TimeStepping;
...
public:
  ...
  /* For temporal coarsening of unsteayd discrete adjoint */
  unsigned short GetUnstAdj_tempcoarse(void);
 ...
 *          */
  ...
  void SetDelta_UnstTime(su2double val_delta_unsttime);
  /*!
  ...
  /*!
   *\brief Get the non-uniform non-dim time step at the ith iteration after restart
   */
  su2double Get_iNonUniform_timestps_ND(unsigned long iter);
  void SetNonUniformTS(void);
  /* Number of non-uniform time steps (length of skip time steps array */
  unsigned long GetnNonUniformTS();
    
  /* Number of skipped flow time steps to compute the next disc adj step */
  su2double Get_iDiscAdjNonUniform_timestps_ND(unsigned long iter);
  void SetDiscAdjNonUniformTS(void);
    
  bool GetNonUniformTimeStepping(void);
  /*!
