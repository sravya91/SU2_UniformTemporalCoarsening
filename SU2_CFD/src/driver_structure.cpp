...
void CDriver::StartSolver() {
  ...
  unsigned long nExtIter_tmp;
  unsigned short tmpcoarse = config_container[ZONE_0]->GetUnstAdj_tempcoarse();

  if ( config_container[ZONE_0]->GetKind_Solver() == DISC_ADJ_RANS || config_container[ZONE_0]->GetKind_Solver() == DISC_ADJ_EULER || config_container[ZONE_0]->GetKind_Solver() ==DISC_ADJ_NAVIER_STOKES)
    {
      cout << "DIscrete adjoint temporal coarsening **********" << endl;
      nExtIter_tmp = config_container[ZONE_0]->GetnExtIter()/tmpcoarse;
    }
  else
    nExtIter_tmp = config_container[ZONE_0]->GetnExtIter();
  
  cout << "********nExtIter = " << nExtIter_tmp << endl;

  while ( ExtIter < nExtIter_tmp ) {
...
void CFluidDriver::Run() {
  ...
  su2double *SolutionFlow, *SolutionFlow_n, *SolutionFlow_n1;
  unsigned short nVar = solver_container[ZONE_0][MESH_0][FLOW_SOL]->GetnVar();
  su2double Physical_dt = config_container[ZONE_0]->GetDelta_UnstTime();
  unsigned long ExtIter = config_container[ZONE_0]->GetExtIter();
  unsigned long nExtIter = config_container[ZONE_0]->GetnExtIter();
  su2double Truncerr[nExtIter*nVar], val_residual, Res_MaxErr[nVar];
    
    /*--- Store the physical time step ---*/
  su2double nonuniform_timestep_ratio = 1.0; /* = dt_old/dt_new */
  su2double TimeStep=0;
  ...
  if (checkConvergence == nZone) break;
    ...
    if (ExtIter > 3)
    {
        for (unsigned long iPoint =0; iPoint < geometry_container[ZONE_0][MESH_0]->GetnPoint(); iPoint++) {
            ...
            //Adjoint solution at iPoint for (n+1) after iteration, n and (n-1) steps
            SolutionFlow = solver_container[ZONE_0][MESH_0][FLOW_SOL]->node[iPoint]->GetSolution();
            SolutionFlow_n = solver_container[ZONE_0][MESH_0][FLOW_SOL]->node[iPoint]->GetSolution_time_n();
            SolutionFlow_n1 = solver_container[ZONE_0][MESH_0][FLOW_SOL]->node[iPoint]->GetSolution_time_n1();
            
            for (unsigned short iVar =0; iVar < nVar; iVar++)
            {
                /* BDF2- BDF1 as the truncation error */
                val_residual = SolutionFlow[iVar]*0.5 - 1*SolutionFlow_n[iVar] + 0.5*SolutionFlow_n1[iVar];
                val_residual = fabs(val_residual/Physical_dt);

                TimeStep = config_container[ZONE_0]->GetDelta_UnstTimeND();
                
                if (config_container[ZONE_0]->GetNonUniformTimeStepping())  {
                    unsigned long current_iter_from_restart = config_container[ZONE_0]->GetExtIter() - config_container[ZONE_0]->GetUnst_RestartIter();
                    TimeStep = config_container[ZONE_0]->Get_iNonUniform_timestps_ND(current_iter_from_restart);
                    if ( rank == MASTER_NODE && iVar==0)
                    ...
                    if ( current_iter_from_restart == 0)
                        nonuniform_timestep_ratio = Physical_dt/TimeStep;
                    else
                        nonuniform_timestep_ratio = config_container[ZONE_0]->Get_iNonUniform_timestps_ND(current_iter_from_restart-1)/TimeStep;
//                    if ( rank == MASTER_NODE && iVar==0)
//                        cout << "******In driver structure TS - ratio(dt_old/dt_new) = " << nonuniform_timestep_ratio << ", Physical_dt/TimeStep = " << Physical_dt/TimeStep << endl;
                    
                    val_residual = ( (pow((1+nonuniform_timestep_ratio),2) -  1)*SolutionFlow[iVar] - pow((1+nonuniform_timestep_ratio),2)*SolutionFlow_n[iVar] + 1.0*SolutionFlow_n1[iVar]) / (nonuniform_timestep_ratio*(1+nonuniform_timestep_ratio)*TimeStep) - (SolutionFlow[iVar] - SolutionFlow_n[iVar])/TimeStep;
                    
                }
                
                // Finding the max of the residuals across the computational domain
                if (val_residual > Res_MaxErr[iVar]) {
                    Res_MaxErr[iVar] = val_residual;
        ...
        for (unsigned short iVar =0; iVar < nVar; iVar++)
        {
            Truncerr[ExtIter*nVar + iVar ] = Res_MaxErr[iVar];
        }
        if ( rank == MASTER_NODE)
            cout << "FlowError[]=" << Truncerr[ExtIter*nVar+1] << endl;
        
        // Printing the Truncation errors at all times from the last step for adjoint
        
        /*if(rank == MASTER_NODE && ExtIter > (nExtIter-50))
        {
            cout << "Printing the Truncation errors at all times from the last step for flow solution..." << endl;
            for(unsigned long iExtIter =0; iExtIter < nExtIter; iExtIter++)
                cout << Truncerr[ExtIter*nVar+1] << endl;
        }*/
    }
...
void CDiscAdjFluidDriver::Run() {
  ...
  if (((ExtIter+1 >= config_container[ZONE_0]->GetnExtIter()/2) ||
...
void CDiscAdjFluidDriver::SetAdj_ObjFunction(){
  ...
  if (time_stepping){
    if (ExtIter < IterAvg_Obj){
      seeding = tmpcoarse*1.0/((su2double)IterAvg_Obj);
        //cout << "Seeding for uniform tmpcoarse = " << seeding << endl;
        if (config_container[ZONE_0]->GetNonUniformTimeStepping())    {
            su2double TotalSteps = 0;
            su2double currentSkip = config_container[ZONE_0]->Get_iDiscAdjNonUniform_timestps_ND(ExtIter);
            for(unsigned long iNonUniformIter =0; iNonUniformIter < config_container[ZONE_0]->GetnNonUniformTS(); iNonUniformIter++)
                TotalSteps += config_container[ZONE_0]->Get_iDiscAdjNonUniform_timestps_ND(iNonUniformIter);
            seeding = currentSkip/TotalSteps;
            //cout << "seeding in NonUniform TS = " << seeding << endl;
        }
    }
    else{
      seeding = 0.0;
