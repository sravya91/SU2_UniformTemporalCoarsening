...
void CDiscAdjSolver::SetAdjoint_Output(CGeometry *geometry, CConfig *config) {
  ...
  for (iPoint = 0; iPoint < nPoint; iPoint++) {
    ...
    // FWH
    unsigned short tmpcoarse = config->GetUnstAdj_tempcoarse();
    //cout << "In solveradjointdiscrete -------- tmpcoarse = " << tmpcoarse << endl;
    if (KindDirect_Solver == RUNTIME_FLOW_SYS   ){
    if (config->GetExtIter()<config->GetIter_Avg_Objective()){
    if (LocalPointIndex[iPoint] >= 0){
        for (iVar = 0; iVar < nVar; iVar++){
          //Solution[iVar] += tmpcoarse*dJdU_CAA[LocalPointIndex[iPoint]][iVar];  
            if (config->GetNonUniformTimeStepping())    {
                unsigned long currentIter = config->GetExtIter();
                cout << "In dJdU setting tmpcoarse = " << config->Get_iDiscAdjNonUniform_timestps_ND(currentIter) << endl;
                Solution[iVar] += config->Get_iDiscAdjNonUniform_timestps_ND(currentIter)*dJdU_CAA[LocalPointIndex[iPoint]][iVar];
            }
            else
                Solution[iVar] += tmpcoarse*dJdU_CAA[LocalPointIndex[iPoint]][iVar];
         }
