...
void CDiscAdjFluidIteration::Preprocess(COutput *output,
  ...
  if ( rank == MASTER_NODE)
      cout << "******* tmp coarse =  " << tmpcoarse << endl;
   // cout << "ExtIter in preprocess " << ExtIter << endl;
  
  
  if (ExtIter == 0 && !config_container[ZONE_0]->GetNonUniformTimeStepping())
    {
      su2double Physical_dt;
      Physical_dt = config_container[val_iZone]->GetDelta_UnstTimeND();
      config_container[val_iZone]->SetDelta_UnstTimeND(tmpcoarse*Physical_dt);
      Physical_dt = config_container[val_iZone]->GetDelta_UnstTime();
      config_container[val_iZone]->SetDelta_UnstTime(tmpcoarse*Physical_dt);
    }

  /* Non-Uniform TS edit starts */
//    unsigned long current_iter_from_restart=current_iter_from_restart = Direct_Iter - config_container[ZONE_0]->GetUnst_RestartIter();
//    if(config_container[ZONE_0]->GetNonUniformTimeStepping()) {
//        su2double Physical_dt;
//        Direct_Iter = SU2_TYPE::Int(config_container[val_iZone]->GetUnst_AdjointIter()) - tmpcoarse*SU2_TYPE::Int(ExtIter) - 2;
//
//        Physical_dt = config_container[ZONE_0]->GetDelta_UnstTimeND();
//        Physical_dt = config_container[ZONE_0]->Get_iNonUniform_timestps_ND(current_iter_from_restart);
//
//        config_container[val_iZone]->SetDelta_UnstTimeND(tmpcoarse*Physical_dt);
//        Physical_dt = config_container[val_iZone]->GetDelta_UnstTime();
//        config_container[val_iZone]->SetDelta_UnstTime(tmpcoarse*Physical_dt);
//    }
  ...
  if (config_container[val_iZone]->GetUnsteady_Simulation()) {
    ...
    Direct_Iter = SU2_TYPE::Int(config_container[val_iZone]->GetUnst_AdjointIter()) - tmpcoarse*SU2_TYPE::Int(ExtIter) - 2;
      cout << "Default DirectIter = " << Direct_Iter + 1 << endl;
      /* Non-Uniform TS edit starts */
      /* Assuming new list gives skipped flow time steps/ nonuniformdiscadj time stpping from disc adj ext iter numbers (so reverse for flow) */
      if(config_container[ZONE_0]->GetNonUniformTimeStepping()) {
          unsigned long SkipFromLast = 0;
          for (unsigned long i_ExtIter_fromlast=0; i_ExtIter_fromlast < SU2_TYPE::Int(ExtIter); i_ExtIter_fromlast++) {
              SkipFromLast += SU2_TYPE::Int(config_container[ZONE_0]->Get_iDiscAdjNonUniform_timestps_ND(i_ExtIter_fromlast));
          }
          Direct_Iter = SU2_TYPE::Int(config_container[val_iZone]->GetUnst_AdjointIter()) - SkipFromLast -2;
          cout << " Direct Iter in DiscAdj Preprocess for ExtIter = " << ExtIter << " is " << Direct_Iter+1 << endl;
      }
       /* Non-Uniform TS edit ends */
    ...
    if (dual_time_2nd) {
      ...
      if(!config_container[ZONE_0]->GetNonUniformTimeStepping())
          LoadUnsteady_Solution(geometry_container, solver_container,config_container, val_iZone, Direct_Iter-2*tmpcoarse);

        /* Non-Uniform TS edit starts */
      if(config_container[ZONE_0]->GetNonUniformTimeStepping()) {
          cout << "DiscAdj::Preprocess - Loading unsteady solution at (n-2) step which is " << Direct_Iter - SU2_TYPE::Int(config_container[ZONE_0]->Get_iDiscAdjNonUniform_timestps_ND(ExtIter)+config_container[ZONE_0] ->Get_iDiscAdjNonUniform_timestps_ND(ExtIter+1)) << endl;
          
          LoadUnsteady_Solution(geometry_container, solver_container,config_container, val_iZone, Direct_Iter - SU2_TYPE::Int(config_container[ZONE_0]->Get_iDiscAdjNonUniform_timestps_ND(ExtIter)+config_container[ZONE_0] ->Get_iDiscAdjNonUniform_timestps_ND(ExtIter+1)));
    ...
    if (dual_time) {
      ...
      if(!config_container[ZONE_0]->GetNonUniformTimeStepping())
          LoadUnsteady_Solution(geometry_container, solver_container,config_container, val_iZone, Direct_Iter-1*tmpcoarse);
        
        /* Non-Uniform TS edit starts */
        if(config_container[ZONE_0]->GetNonUniformTimeStepping()) {
            cout << "DiscAdj::Preprocess - Loading unsteady solution at (n-1) step which is " << Direct_Iter - SU2_TYPE::Int(config_container[ZONE_0]->Get_iDiscAdjNonUniform_timestps_ND(ExtIter)) << endl;
            
            LoadUnsteady_Solution(geometry_container, solver_container,config_container, val_iZone, Direct_Iter - SU2_TYPE::Int(config_container[ZONE_0]->Get_iDiscAdjNonUniform_timestps_ND(ExtIter)));
        }
        /* Non-Uniform TS edit ends */
    ...
    if ((ExtIter > 0) && dual_time){
      ...
      if(!config_container[ZONE_0]->GetNonUniformTimeStepping())
        ...
        /* Non-Uniform TS edit starts */
        if(config_container[ZONE_0]->GetNonUniformTimeStepping()) {
            cout << "ExtIter > 0 DiscAdj::Preprocess - Loading unsteady solution at (n-2) step which is " << Direct_Iter - SU2_TYPE::Int(config_container[ZONE_0]->Get_iDiscAdjNonUniform_timestps_ND(ExtIter)+config_container[ZONE_0] ->Get_iDiscAdjNonUniform_timestps_ND(ExtIter+1)) << endl;
